...
fetchSingle ∷
  (Monad m, Convertible SqlValue a) ⇒
  String →
  [[SqlValue]] →
  m a
...
setRating conn fRating filmTitle = do
  res ←
    run
      conn
      "UPDATE film SET rating = ? WHERE title = ?"
...
main = withPostgreSQL connString (handleSqlError . demo)
  where
    connString = "host=localhost dbname=sakila_films user=schmidh password=CHANGE_ME"
