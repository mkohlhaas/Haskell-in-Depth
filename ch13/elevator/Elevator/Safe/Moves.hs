...
decideMove ∷ ∀ mx to from. Floor mx to → Floor mx from → Move mx to from
decideMove MkFloor MkFloor =
  case discreteNat ∷ Dec (to :~: from) of
    Yes Refl → StandStill
    No to_neq_from →
      case decideLE ∷ Dec (LEProof to from) of
        Yes to_le_from → withAboveGround to_le_from to_neq_from GoingDown
        No to_gt_from → withLEProof (belowTop to_gt_from) GoingUp
  where
    belowTop ∷ LE to mx ⇒ Neg (LEProof to from) → LEProof (S from) mx
    belowTop neg = leTrans (leSwap neg) leProof
    withAboveGround ∷ LEProof to from → Neg (to :~: from) → (∀ fl. from ~ S fl ⇒ r) → r
